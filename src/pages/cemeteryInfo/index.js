...
const App = () => {
  ...
  const fetchData = async () => {
    // 根据实际情况替换成真实的接口地址
    request
      .get(`/cemetery-info/query/area/${selectedArea || 1}`)
      .then((data) => {
        setData(data)
      })
      .catch((error) => {
            ...
            <Col key={item.id} span={4}>
              <div
                style={{
                  ...
                  height: '120px',
                  padding: '20px',
                  backgroundColor: item.cemeteryStatus ? '#509296' : 'gray',
                  borderRadius: '4px',
                  display: 'flex',
  ...
  const fetchBoxData = async (cemeteryCode) => {
    try {
      const response = await request.get(`/cemetery-info/query/${cemeteryCode}`)
  ...
  const handleFormFinish = (values) => {
    ...
    // 调用购买接口
    request
      .post('/cemetery-info/buy', requestData)
      .then(() => {
